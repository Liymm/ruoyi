<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="feedbackList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="图片路径" align="center" prop="filePath" width="100">
        <template slot-scope="scope">
          <image-preview :src="'http://39.108.163.169:8080'+scope.row.filePath" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column label="发表时间" align="center" prop="issueTime" width="180">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getFeedback(uuid).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除意见反馈编号为"' + uuids + '"的数据项？').then(function() {
        return delFeedback(uuids);
      }).then(() => {
        this.getList();
